...
# user can specify their own fastq file, if not given use the standard one
if [ -z "$1" ]; then 
   INPUT_FILES="/commons/Themas/Thema12/HPC/rnaseq.fastq"
else 
   INPUT_FILES=("$@") # user input
fi
...
for INPUT_FILE in "${INPUT_FILES[@]}"; do
  ...
  parallel -S "${HOSTS}" --pipepart --jobs 4 --block 500M --memfree 1G --recstart "@.*" python3 "${WORK_DIR}"/assignment3.py --calculate :::: "${INPUT_FILE}" | python3 "${WORK_DIR}"/assignment3.py --merge -o "${FILE_NAME}" -n "${NUM_FILES}"
done
